...
class Invoice(models.Model):
    ...
    def get_invoice_with_billing_info(invoice_number: str):
        return Invoice.objects.select_related("billing_info").filter(invoice_number=invoice_number).first()
...
class Programmer(models.Model):
    ...
    # Will have an instance of the Programmer, and will have to get all the projects that he/she is involved in
    # including the technologies used.
    def get_projects_with_technologies(self):
        return Project.objects.prefetch_related("programmers__projects__technologies_used").filter(programmers=self)
...
class Task(models.Model):
    ...
    def search_tasks(query: str):
        conditions = Q(title__contains=query) | Q(description__contains=query)
