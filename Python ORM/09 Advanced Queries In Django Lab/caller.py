...
# annotate / Creates a field that is not stored in the database, in which you can perform various calculations
# and aggregations. Then you can access it to print the results.


# 03 Ordered Products per Customers
def ordered_products_per_customer():

    all_ordered_products_per_customer = Order.objects.prefetch_related(
        "orderproduct_set__product"
    ).all().order_by("id")

    result = []
    for order in all_ordered_products_per_customer:
...
def filter_products():
    ...
    for product in Product.objects.filter(query).order_by("-price", "name"):
        resulting_products.append(f"{product.name}: {product.price}lv.")

    return "\n".join(resulting_products)
