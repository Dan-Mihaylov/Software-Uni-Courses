...
def find_authors_nationalities():
    ...
    information = [f"{a.first_name} {a.last_name} is {a.nationality}" for a in authors]
    return "\n".join(information)
...
def order_books_by_year():
    ...
    result = [
        f"{book.publication_year} year: {book.__str__()}" for book in ordered_books
    ]
    return "\n".join(result)
...
def delete_review_by_id(review_id: int):
    ...
    information = f"Review by {review_to_delete.reviewer_name} was deleted"
...
def filter_authors_by_nationalities(author_nationality: str):
    filtered_authors = Author.objects.filter(nationality=author_nationality).order_by("first_name", "last_name")
    information = [
        f"{a.biography}" if a.biography else f"{a.first_name} {a.last_name}" for a in filtered_authors
    ]
    return "\n".join(information)
...
def filter_authors_by_birth_year(start_year: int, end_year: int):
    ...
    information = [
        f"{a.birth_date}: {a.__str__()}" for a in filtered_authors
    ]
    return "\n".join(information)
...
def change_reviewer_name(rev_name: str, new_name: str):
    changed = Review.objects.filter(reviewer_name=rev_name).update(reviewer_name=new_name)
