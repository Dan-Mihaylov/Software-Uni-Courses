def get_character_best_result(half, char):
    best_result = 0
    current = 0
    for ch in half:
        if ch == char:
...
for ticket in tickets:
    ...
    for symbol in symbols:
        left_part_score = get_character_best_result(left_part, symbol)
        right_part_score = get_character_best_result(right_part, symbol)
