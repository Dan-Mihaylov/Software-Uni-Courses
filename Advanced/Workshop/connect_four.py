# To check the win condition use nested dict, append the values of the matrix, and if there is 4 values length,
# return the winner
...
def implement_player_data_to_main_logic(playground: list, player_index, player):
    start_row_index = 0
...
def check_win_condition(playground, row_index, column_index, win_counter):
    ...
    return any(len(current_list) == win_counter and len(set(current_list)) == 1for current_list in possible_winner_checker)
...
try:
    ...
    game_play_function(playground, 4)
except ValueError:
    print("Invalid Value")